...
description: |
  ...
  TRIGGER: "/agentflow-publish", "blogflow preview", "blogflow publish", "af preview", "Gate C", "Gate D", "image-gate", "publish-rollback", "medium-package", "review-publish-mark", "PR:mark", "I:cover_only", "PD:dispatch".
...
Wraps `blogflow draft-show`, `blogflow image-resolve`, `blogflow image-auto-resolve`, `blogflow preview`, `blogflow publish`, and `blogflow publish-rollback`. Goal: ship `<article_id>` to every configured long-form platform and report status, or take a post back down if the user changes their mind.

**Platform status (v0.1):**
- `medium` — default. Manual package/browser paste is always available; close the loop with `blogflow review-publish-mark <article_id> <url>`.
...
- Twitter and newsletter are **not** part of `blogflow publish`; use `blogflow tweet-*` and `blogflow newsletter-*` instead.
...
```bash
PYTHONPATH=. blogflow draft-show <article_id> --json
```
...
For the **Preferences** line, run `PYTHONPATH=. blogflow prefs-show --json` and
summarise as:
...
5. If **zero** intent tokens appear, flag:
   ...
   /agentflow-write <article_id> / clear intent via `blogflow intent-clear`)
   ```
...
Shortcut: if `blogflow intent-check` is wired, run `PYTHONPATH=. blogflow intent-check <article_id> --json` and use its `alignment_score` (< 0.1 -> drift flag; 0.1-0.5 -> partial note; >= 0.5 -> quiet).
...
Options:
  ...
  (a2) say 'auto' to try `blogflow image-auto-resolve`,
...
```bash
PYTHONPATH=. blogflow image-auto-resolve <article_id> --json
```

If they have a known local library path, include it:

```bash
PYTHONPATH=. blogflow image-auto-resolve <article_id> --library <absolute_dir> --json
```

Show the matched files and remaining unresolved count, then continue with manual `blogflow image-resolve` only for anything still unresolved.

For each path the user gives:

```bash
PYTHONPATH=. blogflow image-resolve <article_id> <placeholder_id> <absolute_path>
```

If they say `strip`, remember this and later pass `--force-strip-images` to `blogflow preview` and `blogflow publish`.
...
```bash
PYTHONPATH=. blogflow preview <article_id> --json
```

For the explicit Medium manual path: `PYTHONPATH=. blogflow preview <article_id> --platforms medium --json`, then `PYTHONPATH=. blogflow medium-package <article_id>`.
...
- "cancel" -> stop. Do not run `blogflow publish`.

## Step 6 — publish

```bash
PYTHONPATH=. blogflow publish <article_id> --platforms <csv> [--force-strip-images] --json
```
...
```bash
PYTHONPATH=. blogflow review-publish-mark <article_id> <published_url>
```
...
If any `status != "success"`, ask: "Retry failed platforms? (yes / no)". On yes, re-run `blogflow publish <id> --platforms <failed_csv>`.
...
```bash
PYTHONPATH=. blogflow publish-rollback <article_id> --json
```
...
```bash
PYTHONPATH=. blogflow publish-rollback <article_id> --post-id <platform_post_id> --json
```
...
**Failure modes**:
- `"no successful <platform> record found"` -> the article was never successfully published to that platform. Tell the user to check `blogflow memory-tail`.
...
- `blogflow publish` fails with "Unresolved image placeholders present." -> means you skipped Step 2 or the user didn't commit to strip. Re-prompt.
- `blogflow publish` fails with "no platform versions found" -> you skipped Step 3 (preview). Run it now.
...
- `drafts/<id>/d3_output.json` — persisted by `blogflow preview`.
- `drafts/<id>/platform_versions/*.md` — persisted by `blogflow preview`.
...
- Never call `agentflow.agent_d3.*` or `agentflow.agent_d4.*` directly — always via `blogflow`.
...
`MOCK_LLM=true blogflow publish <id> --force-strip-images --json` uses mock publishers that return fake URLs (`https://blog.mock/...` for Ghost, `https://www.linkedin.com/feed/update/mock_...` for LinkedIn). Safe for dry runs.
