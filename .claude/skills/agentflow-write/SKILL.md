...
description: |
  ...
  TRIGGER: "/agentflow-write", "blogflow write", "blogflow fill", "blogflow edit", "af write", "write draft", "Gate B", "写稿", "draft", "section edit".
...
Wraps `blogflow write`, `blogflow fill`, `blogflow edit`, `blogflow draft-show`, and `blogflow image-resolve`. Goal: produce a draft the user is happy with, then hand off to `agentflow-publish`.
...
Prefix every `blogflow` call with `PYTHONPATH=.` unless the package is globally installed.
...
```bash
PYTHONPATH=. blogflow write <hotspot_id> --angle <N> --auto-pick --json
```
...
```bash
PYTHONPATH=. blogflow write <hotspot_id> --angle <N> --json
```
...
```bash
PYTHONPATH=. blogflow fill <article_id> --title 2 --opening 0 --closing 1 --json
```
...
2. `section N paragraph M <cmd>` → `blogflow edit <id> --section N --paragraph M --command "<cmd>"`.
3. `section N <cmd>` → `blogflow edit <id> --section N --command "<cmd>"` (paragraph_index null).
4. `全文 <cmd>` → advanced: iterate `blogflow edit <id> --section N --command "<cmd>"` for every N in the draft. Warn the user this is a destructive bulk edit before running.
5. `regen` → ask for new title/opening/closing indices, then call `blogflow fill <id> --title X --opening Y --closing Z --json`.
...
Re-`Read ~/.agentflow/drafts/<article_id>/draft.md` (or parse the JSON returned by `blogflow edit --json`) and show the user **only the edited section**, with old word count → new word count. Do not re-dump the whole draft.
...
```bash
PYTHONPATH=. blogflow propose-images <article_id> --json
```
...
- Re-syncs `metadata.json::image_placeholders` so `blogflow draft-show` picks up the
  new ones.
...
- **Reject [N]** → manually edit `draft.md` to remove that `[IMAGE: ...]` line,
  then `Read` it again.
- **Regenerate** → re-run `blogflow propose-images <article_id> --json`. It
...
- `blogflow write` fails with "Hotspot not found" → user probably typo'd, or the relevant archive was not created in this `AGENTFLOW_HOME`; re-run `blogflow article-hotspots` / `blogflow search`, then check both `~/.agentflow/hotspots/` and `~/.agentflow/search_results/`.
- `blogflow edit` fails with IndexError → section or paragraph is out of range; `blogflow draft-show` to see real indices.
- Any non-zero exit: `tail -n 20 ~/.agentflow/logs/agentflow.log` and show.

## State side-effects

Every run of `blogflow write` creates a new `article_id`. If the user aborts mid-flow, old drafts stay under `~/.agentflow/drafts/<id>/` — safe but clutters the queue. You may mention this once.

Memory events written by this flow: `article_created`, `fill_choices`, `section_edit`, `images_proposed`, `image_resolved`.

## NEVER

- Never call `agentflow.agent_d2.*` Python modules directly. Only via `blogflow`.
- Never modify `draft.md` or `metadata.json` by hand — always go through `blogflow edit` / `blogflow fill`.
- Never invoke another skill via the Skill tool mid-loop; just continue the conversation.

## MOCK_LLM

`MOCK_LLM=true` makes `blogflow write --auto-pick` deterministic (same 5 sections, same 5 image placeholders) — useful for testing skills. Drop it for real writing.
