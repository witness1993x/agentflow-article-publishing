...
```bash
blogflow skill-install      # Claude Code 默认
# 或 Cursor 用户：
blogflow skill-install --cursor
```
...
- **交互向导**（推荐首次部署）：`blogflow onboard`，按 10 个 sections 逐项填。
- **手填**：复制 `.env.example` 到 `.env`，编辑后跑 `blogflow doctor` 校验。

`blogflow doctor` 全绿（含第 13 项 `daemon_liveness` 暂时 N/A）才进入下一步。

## 5. Profile + Style bootstrap（per-profile 用户）

```bash
# 方式 A：交互向导
blogflow topic-profile init -i --profile <id>

# 方式 B：从 patch 文件
blogflow topic-profile init --profile <id> --from-file <patch.yaml>
...
示例 unit 文件：`agentflow-deploy/blogflow-review.service`（同目录）。

```bash
sudo cp agentflow-deploy/blogflow-review.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now blogflow-review.service
systemctl status blogflow-review.service
```
...
```bash
blogflow review-cron-install --times "09:00,18:00"
```

Linux 下建议使用 `BLOGFLOW_ARTICLE_HOTSPOTS_SCHEDULE="09:00,18:00"` 由 daemon 内置 scheduler 触发。
...
- daemon 心跳：`blogflow doctor` 第 13 项 `daemon_liveness`，读 `~/.agentflow/review/last_heartbeat.json`
- 实时日志：`journalctl -u blogflow-review -f`
- 失败重启：unit 已配 `Restart=on-failure` + `RestartSec=10`
- 状态快照：`systemctl status blogflow-review.service`

## 9. 升级

```bash
# 拉取新代码
cd /home/blogflow/agentflow-article-publishing && git pull
...
# 重启 daemon
sudo systemctl restart blogflow-review.service
```
...
| 症状 | 处置 |
|---|---|
| `preflight failed` | `blogflow doctor`，按红项逐条修 |
| daemon stale | `journalctl -u blogflow-review -n 50` |
| chat_id missing | 在 TG 给 bot 发 `/start`（首次自动捕获） |
| callback 已失效 | 等下一轮，或 `blogflow review-post-{b,c,d} <aid>` 手工补卡 |
| gate D 卡死 | `blogflow review-resume <aid> --to-state image_approved` 强转 |
