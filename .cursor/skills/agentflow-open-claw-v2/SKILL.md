...
兼容版本：本 skill v3.0 与 `blogflow-lark-deploy-v1.3.3.tar.gz` 及更新版本配合。**v1.3.0 (Phase 3) 起 Telegram surface 已彻底删除**;v1.3.1 删 render.py + 修 timeout sweeper bug;**v1.3.2 加了 Agent-Lark Window 模式**(无 webhook,daemon 写文件队列 + skill agent tail + 挂载 Lark 窗口推卡);v1.3.3 补齐运维交付物——smoke-test CLI (`blogflow agent-events-emit-test`)、卡片 schema 内置 `references/lark_review_cards.md`、单页云电脑部署 runbook (`docs/CLOUD_COMPUTER_DEPLOY.md`)。v3.0 在 v2.9 基础上加入 Phase 1 完成的 7 张新卡的渲染契约：
...
- 完整运维步骤(从云电脑无 sudo 装 Python 一路到起 daemon)看仓库 `docs/CLOUD_COMPUTER_DEPLOY.md`。
